...
void game(Tictactoe &field)
{
 ...
 bool vvod = true;
	while (running) {
		do {
		  if(vvod == false){
		    cout<<"Ievadiet vēlreiz skaitlis\n";
		  }

			field.print(field);

			if (player == 'x') {
			    cout<<"Spēlētājs X piegājienus\n";
			}
			else {
			    cout<<"Spēlētājs O piegājienus\n";

			}

			cout<<"Ievadiet traversa x y koordinātas (atdalot ar atstarpi):";
			cin >> i >> j;
...
int main() 
{
  setlocale(LC_ALL, "Lat"); 
  
  int size;
	 cout<<"Ievadiet lauka lielumu: ";
	 cin >> size;
